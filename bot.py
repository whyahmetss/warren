...
DEEPSEEK_API_KEY = os.environ.get(“DEEPSEEK_API_KEY”, “sk-0067f8e79de84fa88c981c0a331ee14a”)
...
def deepseek_sor(prompt, max_tokens=1500):
“”“Deepseek API ile analiz yaptir”””
...
log.error(f”Deepseek hata: {data}”)
return None
except Exception as e:
log.error(f”Deepseek istegi hatasi: {e}”)
return None
...
minute = f”{int(parts[1]):02d}” if len(parts) > 1 else “00”
except: pass
...
“”“HTF mumlari 30 dakika cache”””
global *htf_cache
now = datetime.utcnow()
key = f”{symbol}*{interval}”
if key in _htf_cache:
ts, df = _htf_cache[key]
if (now - ts).total_seconds() < 30 * 60:  # 30 dk cache
return df
...
prompt = f"""Sen bir ICT (Inner Circle Trader) piyasa analiz uzmanisın. {symbol_display} için bugun ({tarih}) gunluk HTF analiz yaz.
```
...
📍 ZONE 1: [aralik] - [OB/FVG/Breaker] - [aciklama]
⚠️ Invalid: [seviye] body close
📍 ZONE 2: [aralik] - [OB/FVG/Breaker] - [aciklama]
━━━━━━━━━━━━━━━━━━━━━━
🎯 LIKIDITE HEDEFLER
1️⃣ [hedef 1]
2️⃣ [hedef 2]
...
━━━━━━━━━━━━━━━━━━━━━━
✅ Bu bir analiz, sinyal degil. Risk max %1-2
📊 Bir sonraki analiz yarin 09:00’da”””

```
return deepseek_sor(prompt, max_tokens=2000)
```
...
```
while True:
    await asyncio.sleep(180)  # 3 dk - API limit optimizasyonu
...
if cmd in ("durum", "durum_bot"):
    ...
    sig = analyze_ict(df_ltf, df_htf)
    await reply(format_signal(sym, sig) if sig else f"{sym}: Setup yok, bekleniyor...")
elif cmd == "fiyat":
    lines = ["=== FIYATLAR ==="]
    for symbol, cfg in SYMBOLS.items():
        p = get_price(symbol)
    ...
    aktif_list = [f"  {SYMBOLS.get(sym,{}).get('name',sym)} {s['direction']}" for sym, s in aktif_sinyaller.items()]
    ...
    await reply(
        ...
        f"Son 10: {son10}\n\n"
        f"{chr(10).join(perf_lines) if perf_lines else '  —'}\n\n"
        f"Aktif:\n{aktif_text}"
    )
elif cmd == "equity":
    uid = q.from_user.id
    ...
    analiz = deepseek_sor(
        ...
        "Kisa ve ICT perspektifinden yaz.",
        max_tokens=600
    )
    await reply(f"📰 Haber Analizi\n\n{analiz}" if analiz else "Haber analizi alinamadi.")
...
sig_id = f”{symbol.replace(’/’, ‘*’)}*{int(datetime.utcnow().timestamp())}”
...
else: await update.message.reply_text(“Geçersiz komut. `/pnl` yaz.”, parse_mode=“Markdown”)
...
f”Lot: `{lot}` | P/L: `${pnl:+.2f}`”, parse_mode=“Markdown”)
except Exception as e:
...
kayitlar = pnl_db.get(uid, [])
if not kayitlar:
await update.message.reply_text(“Henuz islem yok. `/pnl ekle` ile ekle.”, parse_mode=“Markdown”); return
...
satirlar.append(f”{‘✅’ if k[‘pnl’] > 0 else ‘❌’} {k[‘sembol’]} {k[‘yon’]} `${k['pnl']:+.2f}`”)
satirlar.append(f”\n💰 Toplam: `${toplam:+.2f}`”)
...
satirlar.append(f”{‘✅’ if k[‘pnl’] > 0 else ‘❌’} {k[‘tarih’]} {k[‘sembol’]} {k[‘yon’]} `${k['pnl']:+.2f}`{sebep}”)
await update.message.reply_text(”\n”.join(satirlar), parse_mode=“Markdown”)
...
# ── FAVORİ, ALARM, SEANS, EQUITY, DASHBOARD ─────────────────
...
f”Son 10: {son10}\n\n”
f”━━━ SEMBOL BAZLI ━━━\n”
f”{chr(10).join(perf_lines) if perf_lines else ’  Henuz islem yok’}\n\n”
f”━━━ AKTİF SİNYALLER ━━━\n”
f”{chr(10).join(aktif_lines) if aktif_lines else ’  Yok’}\n\n”
f”━━━ AYARLAR ━━━\n”
...
“Piyasaları etkileyen güncel haberleri ve makroekonomik ortamı değerlendir. “
...
await update.message.reply_text(“❌ Analiz alinamadi. DEEPSEEK_API_KEY kontrol et.”)
...
for symbol, sig in aktif_sinyaller.items():
    # Candle'dan fiyat al - get_price yerine
    df_son = get_candles(symbol, SYMBOLS[symbol]["interval"], 1)
    price = float(df_son.iloc[-1]["c"]) if df_son is not None else None
    if not price:
        continue
...
sym_map = {“XAUUSD”: “XAU/USD”, “US100”: “QQQ”, “NAS100”: “QQQ”, “EURUSD”: “EUR/USD”}
...
await update.message.reply_text(
f”📊 *Backtest Raporu - {SYMBOLS.get(symbol,{}).get(‘name’,symbol)}*\n”
f”{’=’*24}\n”
...
f”Net R        : {toplam_rr:+.1f}R\n”
f”{’=’*24}\n”
f”Son 10: {son10}”, parse_mode=“Markdown”)
except Exception as e:
...
kz_name = kz[“name”]; start_h = kz[“start”]; end_h = kz[“end”]
...
satirlar = [“📊 *Gunluk Ozet*”, f”Toplam: {stats[‘total’]} | ✅{stats[‘win’]} ❌{stats[‘loss’]} | WR: %{wr:.1f}”]
for sym, s in stats_per_symbol.items():
if s[“total”] > 0:
swr = s[“win”] / s[“total”] * 100
...
if stats[“total”] == 0: return
wr = stats[“win”] / stats[“total”] * 100
satirlar = [“📈 *Haftalik Ozet*”, f”Toplam: {stats[‘total’]} | ✅{stats[‘win’]} ❌{stats[‘loss’]} | WR: %{wr:.1f}”]
for sym, s in stats_per_symbol.items():
if s[“total”] > 0:
swr = s[“win”] / s[“total”] * 100
satirlar.append(f”  {SYMBOLS.get(sym,{}).get(‘name’,sym)}: {s[‘total’]} islem WR %{swr:.1f}”)
